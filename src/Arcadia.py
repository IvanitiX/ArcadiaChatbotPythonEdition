    ...
    async def duelo(self,ctx: commands.Context):
        ...
        if ((round(time.time()) - self.TiempoAnterior) >= 60):
            if (ctx.message.content != "!duelo"):
                contrincante = split(string=ctx.message.content,pattern=" ",maxsplit=1)
                await ctx.send(f"/me Atención, {ctx.author.display_name} ha retado a un duelo a {contrincante[1]}, preparense.")
                ...
                if (ataque == 0):
                    await ctx.send(self.victoria(ctx.author.display_name, contrincante[1]))
                    duelista1 += 1
                elif (ataque == 1):
                    await ctx.send(self.derrota(ctx.author.display_name, contrincante[1]))
                
                await ctx.send(f"/me Contraataque: ")
                if (contrataque == 0):
                    await ctx.send(self.victoria(contrincante[1], ctx.author.display_name))
                    duelista2 += 1
                elif (contrataque == 1):
                    await ctx.send(self.derrota(contrincante[1], ctx.author.display_name))
                    
                if  (duelista1 > duelista2):
                    await ctx.send(f"/me {ctx.author.display_name} es el ganador de este duelo")
                elif (duelista1 < duelista2):
                    await ctx.send(f"/me {contrincante[1]} es el ganador de este duelo")
                else:
                    await ctx.send(f"/me Al parecer se han tumbado el uno al otro... ¡Doble KO!")
                self.TiempoAnterior = round(time.time())
            else:
                await ctx.send(f"/me No puedes retar a todo el chat, necesito que digas a quien quieres enfrentarte.")
